...
function GameState:cloneObject(object, alreadyCloned)
    local newObject = {}
    for key, value in pairs(object) do
        local toSave = value
        if type(value) == "table" and not alreadyCloned[value] then
            alreadyCloned[value]= true
            toSave = self:cloneObject(value, alreadyCloned)
